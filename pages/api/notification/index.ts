...
export default withIronSessionApiRoute(notificationRoute, sessionOptions);

async function notificationRoute(req: NextApiRequest, res: NextApiResponse) {
    if (req.method !== "POST" || !req.session.user) {
        return res.status(403).json({message: "Access denied"});
    }
    try {
        const {page} = req.body
        let code = await axios.post(fnApiUrl('3.0/notification?mode=notify'), {
            area: "A",

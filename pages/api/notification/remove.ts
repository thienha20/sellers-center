...
export default withIronSessionApiRoute(removeNotificationRoute, sessionOptions);

async function removeNotificationRoute(req: NextApiRequest, res: NextApiResponse) {
    if (req.method !== "POST" || !req.session.user) {
        return res.status(403).json({message: "Access denied"});
    }
    try {
        const {notification_id} = req.body
        let code = await axios.post(fnApiUrl('3.0/notification?mode=remove_notify'), {
            area: "A",

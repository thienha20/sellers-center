...
const GraphicWeekOrders: FC = () => {
    ...
    useEffect(() => {
        const call_api = () => {
            return axios.post(fnCurrentApiUrl("/api/orders/graphic/week"), {})
        }
        call_api().then(rs => {
    ...
    return (<div className="card card-xxl-stretch mb-5 mb-xl-8">
            <div className="card-header border-0 pt-5">
                <h3 className="card-title align-items-start flex-column">
                    <span className="card-label fw-bolder fs-3 mb-1">{intl.formatMessage({id: 'LANGUAGE.STATISTICS_IN_WEEK'})}({fnPriceFormat(total)})</span>
                </h3>
            </div>
